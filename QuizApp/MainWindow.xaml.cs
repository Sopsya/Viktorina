...
namespace QuizApp
{
    public partial class MainWindow : Window
    {
        ...
        {
            var createQuizWindow = new SozdatViktorinu();
        ...
        private void StartQuizButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                var quizPlayWindow = new Viktorina(selectedQuiz);
                quizPlayWindow.QuizCompleted += OnQuizCompleted;
                quizPlayWindow.ShowDialog();
            }
            else MessageBox.Show("Выберите викторину для прохождения!");
        ...
        private void OnQuizCompleted(PlayerScore score)
        {
            ...
            PlayerScores.Clear();
            foreach (var sortedScore in sortedScores) PlayerScores.Add(sortedScore);
        ...
        {
            var leaderboardWindow = new Rating(PlayerScores);
        ...
        private void EditQuiz_Click(object sender, RoutedEventArgs e)
        {
            if (QuizListBox.SelectedItem is Quiz selectedQuiz)
            {
                var editWindow = new RedaktirovanieQuiz(selectedQuiz);
            ...
            else MessageBox.Show("Выберите викторину для редактирования.", "Ошибка", MessageBoxButton.OK, MessageBoxImage.Warning);
            
        }

        private void DeleteQuiz_Click(object sender, RoutedEventArgs e)
        {
            if (Quizzes.Count == 0) MessageBox.Show("Все викторины удалены. Список пуст.", "Информация", MessageBoxButton.OK, MessageBoxImage.Information);
            ...
            else MessageBox.Show("Выберите викторину для удаления.", "Ошибка", MessageBoxButton.OK, MessageBoxImage.Warning);
